...
// @version      1.3.0
// @description  디시인사이드 모바일/PC 사용자 수동 차단 (사이트 푸터 설정 버튼, 차단 항목 완전 숨김)
...
(() => {
  ...
  function parseEntry(raw, selectedType) {
    ...
    if (selectedType === 'uid') {
      ...
      if (!value) return null;
      return { type: 'uid', value, label: `@${value}` };
    }

    if (selectedType === 'nick') {
      if (value === 'ㅇㅇ') {
        return { error: 'ㅇㅇ은 여러 유동 사용자가 함께 쓰는 닉네임입니다. 유동 IP로 등록해 주세요.' };
      }
    ...
    if (value === 'ㅇㅇ') {
      return { error: 'ㅇㅇ만 입력하면 모든 ㅇㅇ 사용자를 구별할 수 없습니다. ㅇㅇ(211.245)처럼 IP를 함께 입력해 주세요.' };
    }
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      <div class="dcub-box">
        ...
        <div class="dcub-entry">
          ...
          <p id="dcub-help"><b>ㅇㅇ 같은 유동닉은 닉네임이 아니라 IP로 구별합니다.</b> 예: ㅇㅇ(211.245)</p>
        </div>
    ...
    const hints = {
      auto: ['ㅇㅇ(211.245) 또는 @고닉ID', '<b>ㅇㅇ 같은 유동닉은 닉네임이 아니라 IP로 구별합니다.</b> 예: ㅇㅇ(211.245)'],
      ip: ['211.245 또는 ㅇㅇ(211.245)', '화면에 표시된 유동 IP를 입력합니다. 같은 IP 대역 사용자가 함께 차단될 수 있습니다.'],
      uid: ['고닉의 갤로그 ID', '닉네임이 ㅇㅇ이어도 고닉 ID가 다르면 서로 구별할 수 있습니다.'],
  ...
  function findSiteFooter() {
    ...
    return [...candidates]
      ...
      .sort((a, b) => b.score - a.score || b.element.compareDocumentPosition(a.element) & Node.DOCUMENT_POSITION_FOLLOWING ? 1 : -1)[0]?.element || null;
  }
  ...
  function openPanel() {
    ...
    window.setTimeout(() => panel.querySelector('#dcub-input')?.focus(), 50);
  }
  ...
  function scheduleFooterMount() {
    clearTimeout(footerTimer);
    footerTimer = window.setTimeout(mountFooterGear, 180);
  }
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #dcub-bottom-settings { display:none!important; }
      .dcub-site-footer { position:relative!important; }
      #dcub-footer-settings { position:absolute!important;right:14px!important;top:52%!important;transform:translateY(-50%)!important;z-index:3!important;width:38px!important;height:38px!important;display:flex!important;align-items:center!important;justify-content:center!important; }
      #dcub-open { all:unset!important;box-sizing:border-box!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;width:34px!important;height:34px!important;border-radius:8px!important;color:rgba(255,255,255,.68)!important;background:rgba(255,255,255,.06)!important;cursor:pointer!important; }
      #dcub-open:active { color:#fff!important;background:rgba(255,255,255,.16)!important; }
      ...
      #dcub-panel #dcub-type { flex:0 0 82px!important;height:38px!important;box-sizing:border-box!important;padding:0 5px!important;border:1px solid #cfd1da!important;border-radius:6px!important;background:#fff!important;color:#333!important;font-size:12px!important; }
      #dcub-panel #dcub-input { flex:1!important;min-width:0!important;height:38px!important;box-sizing:border-box!important;padding:8px 9px!important;border:1px solid #cfd1da!important;border-radius:6px!important;background:#fff!important;color:#222!important;font-size:13px!important;outline:none!important; }
      #dcub-panel #dcub-input:focus { border-color:#3b4890!important; }
      #dcub-panel #dcub-add { flex:0 0 50px!important;height:38px!important;border:0!important;border-radius:6px!important;background:#3b4890!important;color:#fff!important;font-size:13px!important;font-weight:700!important;cursor:pointer!important; }
      #dcub-panel .dcub-entry p { margin:8px 1px 0!important;color:#858895!important;font-size:11px!important;line-height:1.5!important; }
      ...
      @media(max-width:380px){#dcub-panel #dcub-type{flex-basis:72px!important;font-size:11px!important}#dcub-panel #dcub-add{flex-basis:46px!important}}
      @media(prefers-color-scheme:dark){#dcub-panel .dcub-box{background:#252525!important;color:#eee!important}#dcub-panel header,#dcub-panel footer,#dcub-panel .dcub-entry,#dcub-panel .dcub-item{border-color:#444!important}#dcub-panel #dcub-input,#dcub-panel #dcub-type{background:#303030!important;color:#eee!important;border-color:#5b5d65!important}#dcub-panel .dcub-item button,#dcub-panel .dcub-clear{background:#333!important;color:#eee!important;border-color:#666!important}}
    `;
